...
│                     Template Rendering                       │
│                                                             │
│  Controller                EEx Engine           Browser     │
│  ┌──────────┐         ┌──────────────────┐    ┌─────────┐  │
│  │ render() │────────▶│ profile.html.eex │───▶│  HTML    │  │
│  │          │  assigns│                  │    │  Page    │  │
│  │ name: .. │  ──────▶│ <%= @name %>     │    │         │  │
│  │ id: 42   │         │ <%= @id %>       │    │         │  │
│  └──────────┘         └──────────────────┘    └─────────┘  │
...
│ EEx Tag Types                                      │
│                                                    │
│ <%= expr %>   Output tag ── evaluate and INSERT    │
│                              result into output    │
│                                                    │
│ <%  expr %>   Silent tag ── evaluate but DON'T     │
│                              insert (control flow) │
│                                                    │
│ @name         Shortcut  ── expands to              │
│                              assigns[:name]        │
└────────────────────────────────────────────────────┘
```
