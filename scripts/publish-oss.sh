...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/elixir-byowf-phoenix-liveview.git}"
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/elixir-byowf-phoenix-liveview.git $0 --push"
fi
...
# Read the ignore list into memory first (the file itself may be in the list)
mapfile -t IGNORE_PATHS < "$OSSIGNORE_FILE"
for line in "${IGNORE_PATHS[@]}"; do
